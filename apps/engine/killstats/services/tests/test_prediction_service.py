...
class TestPredictionService:
    ...
    def setup(self) -> None:
        ...
        self.config = CreatureConfig.objects.create(
            creature=self.creature, is_active=True, min_interval=10, max_interval=20
        )

    def _create_event(self, days_ago: int, creature: Creature | None = None) -> None:
        """Helper para criar eventos no passado com suporte a criaturas opcionais."""
    ...
    def test_status_collecting_data_no_event(self) -> None:
        """Caso a criatura nunca tenha sido visto no mundo."""
        result = PredictionService.get_prediction(self.creature, self.world)
    ...
    def test_status_no_chance(self) -> None:
        """Criatura morreu há 5 dias, janela mínima é 10."""
    ...
    def test_status_collecting_no_config_record(self) -> None:
        ...
        new_creature = Creature.objects.create(name="morgaroth-test-service")
        # Não criamos CreatureConfig para ele
        result = PredictionService.get_prediction(new_creature, self.world)
